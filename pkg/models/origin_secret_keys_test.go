...
func testOriginSecretKeyToOneOriginUsingOriginName(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadOriginName(ctx, tx, false, (*[]*OriginSecretKey)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testOriginSecretKeyToOneSetOpOriginUsingOriginName(t *testing.T) {
	var err error
...
func testOriginSecretKeyToOneRemoveOpOriginUsingOriginName(t *testing.T) {
	var err error
...
var (
	originSecretKeyDBTypes = map[string]string{`ID`: `bigint`, `OwnerID`: `bigint`, `Name`: `text`, `Revision`: `text`, `FullName`: `text`, `Body`: `bytea`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `Origin`: `text`}
	_                      = bytes.MinRead
)
