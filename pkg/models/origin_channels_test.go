...
func testOriginChannelToOneOriginUsingOrigin(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadOrigin(ctx, tx, false, (*[]*OriginChannel)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testOriginChannelToOneSetOpOriginUsingOrigin(t *testing.T) {
	var err error
...
func testOriginChannelToOneRemoveOpOriginUsingOrigin(t *testing.T) {
	var err error
...
var (
	originChannelDBTypes = map[string]string{`ID`: `bigint`, `OwnerID`: `bigint`, `Name`: `text`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `OriginName`: `text`}
	_                    = bytes.MinRead
)
