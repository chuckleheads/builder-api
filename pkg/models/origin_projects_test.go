...
func testOriginProjectToOneOriginUsingOrigin(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadOrigin(ctx, tx, false, (*[]*OriginProject)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testOriginProjectToOneSetOpOriginUsingOrigin(t *testing.T) {
	var err error
...
func testOriginProjectToOneRemoveOpOriginUsingOrigin(t *testing.T) {
	var err error
...
var (
	originProjectDBTypes = map[string]string{`ID`: `bigint`, `OriginName`: `text`, `PackageName`: `text`, `Name`: `text`, `PlanPath`: `text`, `OwnerID`: `bigint`, `VCSType`: `text`, `VCSData`: `text`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `VCSInstallationID`: `bigint`, `Visibility`: `enum.origin_package_visibility('public','private','hidden')`, `AutoBuild`: `boolean`}
	_                    = bytes.MinRead
)
