...
var (
	jobDBTypes = map[string]string{`ID`: `bigint`, `OwnerID`: `bigint`, `JobState`: `text`, `ProjectID`: `bigint`, `ProjectName`: `text`, `ProjectOwnerID`: `bigint`, `ProjectPlanPath`: `text`, `VCS`: `text`, `VCSArguments`: `ARRAYtext`, `NetErrorCode`: `integer`, `NetErrorMSG`: `text`, `SchedulerSync`: `boolean`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `BuildStartedAt`: `timestamp with time zone`, `BuildFinishedAt`: `timestamp with time zone`, `PackageIdent`: `text`, `Archived`: `boolean`, `Channel`: `text`, `SyncCount`: `integer`, `Worker`: `text`, `Target`: `text`}
	_          = bytes.MinRead
)
