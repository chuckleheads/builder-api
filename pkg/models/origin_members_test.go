...
func testOriginMemberToOneOriginUsingOriginName(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadOriginName(ctx, tx, false, (*[]*OriginMember)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testOriginMemberToOneSetOpOriginUsingOriginName(t *testing.T) {
	var err error
...
func testOriginMemberToOneRemoveOpOriginUsingOriginName(t *testing.T) {
	var err error
...
var (
	originMemberDBTypes = map[string]string{`ID`: `integer`, `Origin`: `text`, `AccountID`: `bigint`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`}
	_                   = bytes.MinRead
)
